...
df_dummies <- subset(df_dummies, select = -c(discovery_year, fire_size_class, state_WY, discovery_month_Sep, vegetation_type_16, remoteness, fire_cause_Smoking))

respuesta = data_fires$remoteness

lm_fire_size = lm(formula = respuesta ~ ., data = df_dummies)
summary(lm_fire_size)

plot(lm_fire_size$residuals, pch = 16, col = "red")
plot(lm_fire_size, 2)
```
...
```{r}
aov_veg = aov(x$fire_size ~ x$vegetation_type)
summary(aov_veg)

par(mfrow = c(1,2))
qqnorm(x[x$vegetation_type == "1","fire_size"], main = "Type 1")
qqline(x[x$vegetation_type == 1,"fire_size"])
qqnorm(x[x$vegetation_type == 1,"fire_size"], main = "Type 1")
qqline(x[x$vegetation_type == 1,"fire_size"])
```
